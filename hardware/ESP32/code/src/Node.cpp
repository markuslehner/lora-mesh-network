...
void receive_packet() {
    ...
    while (LoRa.available()) {
        ...
        if(!too_long && byte_cnt >= 49) {
            too_long = true;
            Serial.printf("  Received packet is too long, discarding rest of packet!\n");
        }
    }  

    if (too_long) {
        Serial.printf("  Not handling packet!\n");

    } else if(app_id[0] == rec_buffer[0] && app_id[1] == rec_buffer[1] && byte_cnt > 8) {
        ...
        Packet p(rec_buffer[2], rec_buffer[3], rec_buffer[4], rec_buffer[6], rec_buffer[5]>>4, rec_buffer[7]&127, rec_buffer[7]>>7, rec_buffer[8], byte_cnt-9, rec_buffer+9);
        ...
        if(blocked) {
            ...
            // plausibility check
            // payload length in comparison with packet type
            if (p.type == DATA && p.payload_length != 17) {
                Serial.printf("  DATA packet was corrupted!\n");
            } else if (p.type == JOIN_ACK && p.payload_length != 2) {
                Serial.printf("  JOIN_ACK packet was corrupted!\n");
            } else if (p.type == ACK && p.payload_length != 1) {
                Serial.printf("  ACK packet was corrupted!\n");
            } else if (p.type == ROUTE && p.payload_length != 7) {
                Serial.printf("  ROUTE packet was corrupted!\n");
            } else if (p.type == JOIN && p.payload_length != 2) {
                Serial.printf("  JOIN packet was corrupted!\n");
            } else if (p.type == COMMAND  || p.type == COMMAND_ACK ) {
                if(p.payload_length < 1) {
                    Serial.printf("  COMMAND packet was corrupted!\n");
                } else if (p.payload[0] == REQUEST && p.payload_length != 1) {
                    Serial.printf("  REQUEST packet was corrupted!\n");
                } else if (p.payload[0] == SET_INTERVAL && p.payload_length != 18) {
                    Serial.printf("  SET_INTERVAL packet was corrupted!\n");
                } else if (p.payload[0] == RESYNC_INTERVAL && p.payload_length != 5) {
                    Serial.printf("  RESYNC_INTERVAL packet was corrupted!\n");
                } else if (p.payload[0] == SET_DISTANCE && p.payload_length != 2) {
                    Serial.printf("  SET_DISTANCE packet was corrupted!\n");
                } else if (p.payload[0] == SET_BLOCK && p.payload_length != 2) {
                    Serial.printf("  SET_BLOCK packet was corrupted!\n");
                } else if ( !(  p.payload[0] == SET_BLOCK || 
                                p.payload[0] == SET_DISTANCE || 
                                p.payload[0] == RESYNC_INTERVAL || 
                                p.payload[0] == SET_INTERVAL || 
                                p.payload[0] == REQUEST) 
                                && p.payload_length != 1) {
                    Serial.printf("  COMMAND packet was corrupted!\n");
                } else {
        ...
        Serial.printf("  Packet is not from same network\n");
    }    
}
