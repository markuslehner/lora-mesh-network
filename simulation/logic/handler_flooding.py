...
from hw.packet import Payload_type, Command_type, packet, packet_flooding
import numpy as np
import random

class handler_flooding(packet_handler):
    ...
    def relay_packet(self, rx_packet : packet_flooding) -> None:
        ...
        elif(rx_packet.origin == self.node.logic.node_id):
    ...
    def get_num_received(self, rx_packet : packet_flooding) -> int:
        ...
        self.node.debugger.log("[%i] %s" % (rx_packet.origin, self.last_packet_origins), 5)

        for i in range(len(self.last_packet_origins)):
            # self.node.debugger.log("origin: %i/%i" % (self.last_packet_origins[i], rx_packet.origin), 5)
            if(rx_packet.payload_type != Payload_type.ROUTE and self.last_packet_origins[i] == rx_packet.origin ):
